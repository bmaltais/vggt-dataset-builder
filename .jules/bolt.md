...
## 2026-02-19 - [Vectorized Boundary Filtering & Deferred Color Conversion]
**Learning:** Boundary filtering using nested Python loops is a major bottleneck for high-resolution point cloud generation (~O(H) vs O(1) for vectorized slices). Additionally, performing `.astype(np.float32)` on an entire image before filtering wastes massive amounts of memory and CPU cycles on points that will ultimately be discarded.
**Action:** Use vectorized NumPy slicing on reshaped (S, H, W) arrays for boundary filtering; defer expensive type conversions until AFTER all filtering stages to operate only on valid points.
