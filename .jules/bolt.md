...
## 2026-02-19 - [Vectorized Boundary Filtering via View Reshaping]
**Learning:** Filtering boundaries of a flattened (S*H*W) array using nested Python loops is extremely slow (~O(S*H)). Reshaping the 1D mask to (S, H, W) allows using vectorized slice assignments (`mask[:, :t, :] = False`), providing a ~30x-40x speedup while maintaining zero-copy efficiency.
**Action:** Use `mask.reshape(S, H, W)` for efficient spatial filtering of flattened point cloud masks.
