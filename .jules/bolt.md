...
## 2026-06-30 - [Vectorized Point Cloud Filtering]
**Learning:** Nested Python loops for spatial filtering (like boundary checks) are extremely slow for large point clouds (O(S*H)). Vectorizing these operations using 3D view reshapes and slice assignments provides a massive speedup (~10x for boundary filtering). Additionally, using squared distance (`np.einsum`) avoids expensive square root operations (`np.linalg.norm`) in depth filtering.
**Action:** Always vectorize spatial masks using NumPy reshapes and slice assignments; use squared distances to avoid `sqrt` overhead in geometric filters.
