...
**Learning:** Combining multiple point cloud filtering optimizations (vectorized boundary clearing, squared distance depth checks, and in-place mask updates) provides a massive cumulative speedup (~4.4x for 10M points). Vectorizing boundary filtering by reshaping the 1D mask to 3D and using slice assignments is significantly faster than nested CPU loops.
**Action:** Use `mask.reshape(S, H, W)` for vectorized boundary clearing; use squared distance comparisons (`dist_sq <= max_depth**2`) to avoid square root overhead; always use in-place `&=` for boolean mask combinations to minimize allocations.
