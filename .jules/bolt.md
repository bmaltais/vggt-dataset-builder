...
**Learning:** Manual Python loops over image dimensions to create masks (e.g., for boundary filtering) are extremely inefficient (~7x slower) than vectorized NumPy slicing. Reshaping a flattened 1D mask to 3D `(S, H, W)` allows using multi-axis slicing to zero out edges in a single set of operations. Additionally, using in-place `&=` operations for mask combinations reduces memory allocations and improves performance by ~1.6x.
**Action:** Always prefer vectorized slicing on reshaped arrays over manual loops for spatial filtering; use in-place bitwise operators for mask accumulation.
