...
## 2026-02-18 - [Vectorized Boundary Filtering with NumPy Slicing]
**Learning:** Nested Python loops for boundary filtering (setting pixels within N px of the edge to False) are extremely inefficient for large point clouds, even when only processing valid points. Using NumPy slicing on a reshaped (S, H, W) view of the flattened mask provides a >300x speedup. It's critical to use `.copy()` after reshaping to avoid side effects from in-place mutation of potentially shared or cached arrays in node-based environments like ComfyUI.
**Action:** Replace per-pixel loops with vectorized NumPy slicing; use copies when modifying masks in shared environments to avoid side effects.
