...
**Learning:** Nested Python loops for boundary filtering (O(S*H)) are extremely slow on large video sequences. Reshaping the mask to (S, H, W) and using NumPy slice assignments is ~21x faster. Additionally, np.linalg.norm is expensive due to the square root; comparing squared distances (x^2+y^2+z^2 <= d^2) provides a ~4.5x speedup.
**Action:** Use 3D slice assignments for boundary/window masking; always prefer squared distance comparisons over np.linalg.norm for filtering.
