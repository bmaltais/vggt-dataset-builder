...
**Learning:** Python-level nested loops for point cloud boundary filtering are extremely slow on high-resolution datasets (e.g., 1080p). Additionally, using `np.linalg.norm` for max depth filtering introduces unnecessary `sqrt` overhead when a squared distance comparison is mathematically equivalent. Vectorizing the boundary mask with 3D slice assignments and using squared distances provides a ~1.6x-3.5x speedup on 41.5M points.
**Action:** Replace Python loops with NumPy slice assignments for spatial masking; use squared distance comparisons to avoid `sqrt` in performance-critical paths.
