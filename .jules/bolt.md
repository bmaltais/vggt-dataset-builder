...
## 2026-02-18 - [Fast GPU-to-CPU Readback with uint8]
**Learning:** The `HoleFillingRenderer` final pass uses a 3-component `uint8` (`u1`) texture to reduce GPU-to-CPU bandwidth by ~5.3x compared to `float32`. The `_read_final_color` method utilizes `np.frombuffer` and `np.ascontiguousarray(np.flipud(rgb))` to eliminate CPU-side float math, clipping, and casting, achieving a measured ~5.9x speedup in the readback phase and a ~16% overall rendering boost.
**Action:** Move final image processing to the GPU and use uint8 textures for final render outputs to minimize bandwidth and CPU overhead.
