...
## 2026-02-19 - [Vectorized Boundary Filtering in Comfy Nodes]
**Learning:** Nested Python loops for spatial filtering (like boundary masking) across a sequence of frames create a massive O(S*H) bottleneck. NumPy broadcasting allows applying a 2D mask to an entire 3D frame stack (S, H, W) in a single vectorized operation. Additionally, using in-place updates (masking views) avoids large redundant memory allocations.
**Action:** Replace nested image/frame loops with NumPy broadcasting and in-place operations for spatial masking.
