...
## 2026-02-19 - [Vectorized In-Place Masking]
**Learning:** Reshaping a large 1D boolean mask into a multi-dimensional view (e.g., `(S, H, W)`) allows for highly efficient, vectorized in-place modifications using slice assignments. This technique avoids millions of Python-level slice operations in loops and provides a massive speedup (e.g., ~31x for image boundary filtering).
**Action:** Use `reshape()` to create zero-copy views for multi-dimensional spatial filtering on flattened point cloud masks.
