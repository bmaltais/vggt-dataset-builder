...
**Learning:** Applying boundary masks to large point clouds (e.g., 518x518 across multiple frames) using nested Python loops is extremely slow (O(S*H)). Vectorizing this with NumPy slicing on a reshaped mask provides a significant speedup (>7x measured on 100 frames) while being more idiomatic.
**Action:** Use vectorized NumPy slicing for spatial masking operations on images or point clouds to avoid Python loop overhead.
