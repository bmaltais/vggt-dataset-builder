...
**Learning:** Nested Python loops for boundary filtering and using `np.linalg.norm` for distance thresholding are major bottlenecks when processing large point clouds in ComfyUI nodes. Vectorizing the boundary mask with NumPy slice assignments and using squared distances for depth filtering provides a ~3.5x to ~20x combined speedup for these filtering stages.
**Action:** Use vectorized NumPy slice assignments for spatial masking and prefer squared distance comparisons over `np.linalg.norm` to avoid square root overhead.
