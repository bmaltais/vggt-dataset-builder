...
## 2026-02-19 - [Vectorized Boundary Filtering & In-place Masking]
**Learning:** Nested Python loops for 2D boundary filtering (top/bottom/left/right margins) on image batches are extremely inefficient. Reshaping the flattened mask to 3D `(S, H, W)` allows using vectorized NumPy slicing to clear margins in O(1) Python time. Furthermore, all mask combination steps should use in-place `&=` operations on the original flattened mask to eliminate redundant allocations of large boolean arrays, significantly reducing memory pressure and improving processing speed for image sequences.
**Action:** Use 3D reshaping and slicing for batch boundary filtering; prioritize in-place `&=` for mask combinations to keep memory footprint low and execution fast.
