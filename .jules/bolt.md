...
**Learning:** Reading float32 textures from GPU to CPU is bandwidth-heavy (16 bytes/pixel for RGBA) and requires expensive CPU-side clipping and scaling. Moving final image processing to the GPU and using a 3-channel uint8 (u1) texture reduces data transfer by 5.3x and speeds up CPU-side readback processing by ~20x (from ~30ms to ~1.3ms for 1080p).
**Action:** Always use normalized integer textures (u1) for final image readbacks to the CPU; ModernGL/OpenGL handles the 0-1 to 0-255 mapping efficiently on the GPU.
