...
**Learning:** Vectorizing spatial filters (like boundary masks) using slice assignments on reshaped 3D views is ~6x faster than nested Python loops. Additionally, using `np.einsum` for squared distance comparisons avoids the expensive square root overhead of `np.linalg.norm`, providing a ~4x speedup for depth filtering.
**Action:** Always prefer 3D slice assignments over loops for pixel/point-map operations; use squared distance comparisons to avoid unnecessary square roots in filtering logic.
