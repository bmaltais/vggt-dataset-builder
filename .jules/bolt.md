...
## 2026-06-07 - [Vectorized Point Cloud Filtering]
**Learning:** Nested Python loops for boundary filtering and expensive square root operations (`np.linalg.norm`) for depth filtering on millions of points are significant bottlenecks. Vectorizing boundary filtering with NumPy slice assignments on a reshaped view and using squared distance comparisons provides a ~3-5x speedup.
**Action:** Use view-based slice assignments for grid-like filtering and squared distance for radial distance checks to avoid Python loop and math overhead on large arrays.
