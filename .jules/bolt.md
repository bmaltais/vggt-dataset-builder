...
**Learning:** Vectorized boundary filtering for point clouds by creating a 3D mask `(S, H, W)` and applying slice assignments to zero out boundaries avoids expensive nested Python loops and provides a ~9x speedup for high-resolution processing.
**Action:** Replace nested loops performing coordinate-based masking with NumPy slice assignments on multi-dimensional masks.
