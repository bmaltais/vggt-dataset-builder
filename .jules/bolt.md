...
## 2026-02-19 - [Vectorized Boundary Filtering & Optimized PLY Export]
**Learning:** Nested Python loops for 3D array spatial filtering (like boundary cropping) are extremely slow (~250x penalty). Reshaping a 1D mask to 3D and using NumPy slicing is near-instant. Additionally, constructing structured arrays for PLY files can be optimized by using `np.empty` and direct broadcasting of constant scalars (scales, rotations) to columns, avoiding multiple large intermediate array allocations and redundant operations like `np.tile`.
**Action:** Always replace nested spatial loops with NumPy slicing; use broadcasting and structured array views for efficient binary file construction.
