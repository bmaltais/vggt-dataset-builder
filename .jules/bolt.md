...
## 2026-02-18 - [Optimized GPU-to-CPU Readback with uint8]
**Learning:** Reading float32 textures from the GPU to the CPU is 5.3x more bandwidth-intensive than reading uint8 textures. Additionally, performing alpha premultiplication, scaling (* 255.0), and clipping on the CPU for millions of pixels adds significant overhead. Offloading these operations to the fragment shader and using a 3-channel uint8 (u1) texture for readback provides a ~5.9x speedup for the readback phase and a ~16% overall rendering boost.
**Action:** Move final image processing (scaling, clipping) to the GPU and use uint8 textures for final render outputs to minimize bandwidth and CPU-side post-processing.

## 2026-02-18 - [Direct Buffer Initialization with NumPy]
**Learning:** Calling `.tobytes()` on a NumPy array before passing it to ModernGL's `ctx.buffer()` creates an unnecessary copy of the data in Python's memory. ModernGL natively supports the buffer protocol, allowing it to read directly from the NumPy array's memory without the overhead of an extra copy.
**Action:** Pass NumPy arrays directly to `ctx.buffer()` instead of calling `.tobytes()` to reduce redundant memory allocations and improve initialization speed.
