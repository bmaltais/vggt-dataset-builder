...
**Learning:** Iterative boundary filtering with nested Python loops is a massive O(S*H) bottleneck in large point cloud pipelines. Vectorizing this using NumPy slices on a 3D reshaped view of the mask provides a ~31x speedup. Similarly, replacing `np.linalg.norm` with manual squared distance addition for depth filtering avoids expensive square root operations, yielding a ~5x speedup for that stage.
**Action:** Always vectorize image-space filtering operations using NumPy slices; prefer squared distance comparisons to avoid redundant `sqrt` calls in performance-critical paths.
