...
## 2026-02-27 - [Vectorized Boundary Filtering]
**Learning:** Nested Python loops for boundary filtering (iterating over frames and rows) introduce massive overhead when processing large batch sizes and resolutions (e.g., 518x518). Reshaping the flattened mask to `(S, H, W)` and using NumPy's vectorized slicing with the in-place `&=` operator provides a measured ~15x speedup while reducing memory pressure.
**Action:** Replace manual loop-based spatial filtering with vectorized NumPy slicing; use `.ravel()` to return to the expected flat format for downstream indexing.
