...
## 2026-02-19 - [Vectorized Point Cloud Filtering]
**Learning:** Iterative boundary filtering with nested Python loops is a massive bottleneck when processing large batches of video frames (S*H*W). Reshaping the 1D mask into a (S, H, W) view allows for O(1) vectorized slice assignments that are ~10-20x faster. Additionally, using squared distance comparisons for depth filtering avoids expensive square root operations.
**Action:** Always use reshaped views and slice assignments for grid-aligned masks; prefer squared distance comparisons for depth-based thresholding.
