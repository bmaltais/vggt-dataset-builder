...
## 2026-02-19 - [Vectorized In-Place Point Cloud Filtering]
**Learning:** Filtering millions of points using nested Python loops (e.g., for boundary removal) is a massive bottleneck. Vectorizing these operations by reshaping the 1D mask to a 3D view (S, H, W) and using slice assignments provides a ~130x speedup for the filter itself. Additionally, using squared distance comparisons for depth filtering avoids expensive square root operations, and using in-place boolean operators (`&=`) reduces peak memory by avoiding redundant array copies.
**Action:** Always vectorize image-space filtering using NumPy slicing; use squared distances for thresholding; use in-place operations to minimize memory pressure.
