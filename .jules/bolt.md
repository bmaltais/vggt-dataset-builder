...
**Learning:** Reshaping a flattened 1D array to (S, H, W) and using NumPy slicing is significantly faster than nested Python loops for applying boundary masks. Using `.ravel()` instead of `.flatten()` avoids unnecessary memory copies when the array is already contiguous.
**Action:** Prefer vectorized slicing on reshaped arrays over Python loops for image-space masking; use `.ravel()` for memory efficiency.

## 2026-02-19 - [Early Exit for Image Rescaling]
**Learning:** Checking image dimensions via `Image.size` immediately after opening allows for an early return if the image is already within megapixel limits. This avoids expensive and redundant RGBA compositing and RGB conversions.
**Action:** Check metadata (like size) before performing pixel-level operations to enable fast-path exits.
