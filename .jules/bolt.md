...
**Learning:** Nested loops iterating over millions of points or thousands of rows (e.g., for boundary filtering) are a massive bottleneck in Python. Vectorizing these operations using NumPy 3D view slice assignments can achieve speedups of 400x+. Additionally, using squared distance comparisons for depth filtering avoids the expensive square root operation in `np.linalg.norm`, providing an extra ~10x speedup.
**Action:** Always vectorize spatial filtering using array slicing; replace distance-based thresholds with squared-distance comparisons to eliminate `sqrt` overhead.
