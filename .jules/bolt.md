...
## 2026-06-26 - [Vectorized Point Cloud Filtering]
**Learning:** Point cloud filtering operations like boundary checks and depth thresholding are major bottlenecks when implemented with Python loops or redundant math. Vectorizing boundary filtering via 3D view slice assignments provides a ~23x speedup, and using squared distances with `np.einsum` for depth filtering provides a ~4x speedup by avoiding `sqrt` overhead and large intermediate array allocations.
**Action:** Always vectorize spatial filtering operations using NumPy views and avoid `np.linalg.norm` when squared comparisons are sufficient.
