...
## 2026-04-10 - [Vectorized Point Cloud Filtering]
**Learning:** Nested loops for boundary filtering on millions of points are extremely slow in Python. Vectorizing the mask creation as a 2D array and using NumPy broadcasting to expand it to the full batch provides a ~11x speedup. Additionally, using squared distance comparisons for depth filtering avoids expensive square root operations, providing a further ~1.1x boost.
**Action:** Always prefer vectorized NumPy operations and broadcasting over Python-level loops for image-space filtering; use squared distances for radial/depth checks when possible.
