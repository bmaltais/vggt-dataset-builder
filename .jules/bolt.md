...
## 2026-02-19 - [Vectorized Boundary Filtering in ComfyUI Nodes]
**Learning:** Boundary filtering using nested Python loops over frames and image dimensions (O(S * H)) is a significant bottleneck for video sequences. Reshaping the mask to (S, H, W) and using NumPy slicing allows for efficient vectorized operations, providing a measured ~6.8x speedup in this environment (and likely more for larger batches).
**Action:** Always replace manual loops over image coordinates or frame indices with NumPy slicing and boolean masking for batch operations.
