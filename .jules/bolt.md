...
**Learning:** Python loops for image boundary filtering (millions of points) are massive bottlenecks. Reshaping a flat mask into a (S, H, W) view allows using 4 NumPy slice assignments to replace $O(S \times H)$ Python iterations, resulting in a ~10x speedup for the filter. Additionally, avoiding `np.linalg.norm` (sqrt) in favor of squared distance comparisons (`np.einsum`) further reduces overhead.
**Action:** Use reshaped views for spatial filtering on flat point cloud arrays; prefer squared distance comparisons to avoid square roots in hot loops.
