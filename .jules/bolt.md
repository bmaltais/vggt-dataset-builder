...
**Learning:** Nested Python loops for boundary filtering on image-sized masks (S*H*W) are extremely slow (~8-10ms for 10 frames at 518x518). Reshaping the mask to (S, H, W) and using vectorized NumPy slicing reduces this to <0.4ms (>25x overall, >300x for the specific segment), making point cloud generation significantly more responsive.
**Action:** Use vectorized slicing on reshaped arrays to handle boundary or regional masks instead of iterative Python loops.
