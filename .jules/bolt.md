...
## 2026-02-21 - [Vectorized Boundary Filtering & In-place Masking]
**Learning:** Performing spatial filtering (e.g., boundary cropping) on flattened image arrays using nested Python loops is extremely inefficient. Reshaping the mask to its logical dimensions `(S, H, W)` allows using vectorized NumPy slicing, which is significantly faster (~3.8x measured). Furthermore, using in-place logical operators (`&=`) for combining large boolean masks reduces peak memory by avoiding intermediate array allocations.
**Action:** Always prefer reshaping flattened data to use vectorized slicing for spatial operations; use in-place operators for combining large masks to minimize memory pressure.
