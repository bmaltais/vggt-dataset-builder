...
## 2026-02-18 - [Vectorized Boundary Filtering]
**Learning:** Boundary filtering in ComfyUI nodes was using a nested Python loop to generate masks, which is highly inefficient for large point clouds. Reshaping the mask to (S, H, W) and using vectorized NumPy slicing provides a ~4x speedup for typical scenarios (e.g., 518x518 resolution). Additionally, using in-place operators (&=) for mask composition reduces peak memory by avoiding redundant array allocations.
**Action:** Replace nested loops with vectorized slicing whenever possible; use in-place operators for combining large boolean masks to save memory.
