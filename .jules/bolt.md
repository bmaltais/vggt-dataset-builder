...
## 2026-02-19 - [Vectorized Point Cloud Filtering in ComfyUI Nodes]
**Learning:** Iterative O(S*H) Python loops for boundary filtering and redundant square root calculations in depth filtering (`np.linalg.norm`) create massive bottlenecks when processing multi-frame point clouds. Vectorizing boundary masks with 2D broadcasting provides a ~180x speedup, and using squared distance comparisons for depth filtering provides a ~3x speedup.
**Action:** Always use NumPy broadcasting and in-place operations for spatial filtering; replace norm-based distance checks with squared distance comparisons to avoid expensive sqrt operations on large arrays.
