...
## 2026-02-19 - [Vectorized Boundary Masking]
**Learning:** Manually masking image boundaries using nested Python loops is a major bottleneck in point cloud generation pipelines. For a batch of $S$ frames with height $H$, this results in $O(S \times H)$ Python loop iterations. Reshaping the flattened mask to $(S, H, W)$ and using NumPy slicing for boundary assignment allows the operation to run entirely in C, providing a ~2.2x speedup and cleaner code.
**Action:** Replace nested loops with vectorized slicing when applying spatial masks to point clouds or images.
