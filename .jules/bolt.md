...
**Learning:** Processing large point cloud masks (e.g., boundary filtering) using nested Python loops is extremely slow (O(S*H)). Vectorizing these operations by reshaping the flat point array into a 3D view (S, H, W) and using NumPy slice assignments provides a ~46x speedup. Similarly, replacing `np.linalg.norm` with squared distance comparisons avoids redundant square root calculations, giving a ~4.7x speedup.
**Action:** Always prefer vectorized slice assignments on reshaped views over iterative loops for image/point cloud masks; use squared distance for depth filtering to avoid `sqrt` overhead.
