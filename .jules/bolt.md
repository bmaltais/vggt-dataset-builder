...
**Learning:** Applying boundary masks using nested Python loops (looping over frames and rows) is a significant bottleneck for high-resolution point cloud generation. Vectorizing this operation using NumPy slicing on a 3D reshaped mask (S, H, W) eliminates the Python loop overhead and can provide a measured speedup of up to 30x.
**Action:** Use vectorized NumPy slicing for mask generation instead of nested Python loops whenever the underlying data structure is a regular grid.
