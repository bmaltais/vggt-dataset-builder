...
## 2026-02-19 - [Vectorized Boundary Filtering and In-place Masking]
**Learning:** Nested Python loops for spatial filtering (like boundary masking) are extremely inefficient in NumPy. Reshaping flattened point clouds to 3D `(S, H, W)` allows for high-speed vectorized slicing, providing a ~15x speedup. Additionally, using in-place boolean operations (`&=`) for mask combination significantly reduces peak memory pressure by avoiding redundant large array allocations.
**Action:** Always prefer vectorized slicing over Python loops for spatial operations; use in-place operators for combining large boolean masks.
