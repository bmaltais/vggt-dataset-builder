...
**Learning:** Nested Python loops for spatial boundary filtering in large point clouds (S*H*W) introduce massive interpreter overhead. Vectorizing this by reshaping the flat mask into its 3D spatial dimensions (S, H, W) and using NumPy slice assignments is ~30-50x faster and significantly more readable.
**Action:** Always prefer NumPy slicing on reshaped views over explicit loops for spatial masking operations.
