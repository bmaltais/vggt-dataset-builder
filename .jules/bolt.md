...
## 2026-05-08 - [Vectorized Point Cloud Filtering]
**Learning:** Python loops for per-frame or per-pixel coordinate filtering (like boundary checks) are massive bottlenecks in point cloud processing. Reshaping flattened point masks into 3D structures allows for O(1) vectorized slice assignments in NumPy, providing a ~200x speedup. Additionally, replacing `np.linalg.norm` with squared distance comparisons avoids the expensive square root operation, yielding another ~6x gain in depth filtering.
**Action:** Always vectorize spatial and coordinate-based filtering using NumPy slices or broadcasting instead of iterative Python loops.
