...
**Learning:** Nested Python loops for boundary masking are extremely slow for high-resolution images or large frame batches. Reshaping the mask to (S, H, W) and using NumPy slicing to apply boundary values provides a ~16x speedup compared to manual row-by-row iteration.
**Action:** Always prefer NumPy slicing over manual coordinate loops for spatial masking operations on large arrays.
