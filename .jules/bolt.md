...
## 2026-06-05 - [Vectorized Point Cloud Filtering]
**Learning:** Replaces nested Python loops in boundary filtering with vectorized NumPy slice assignments on a 3D view of the mask, and uses manual squared distance calculation instead of `np.linalg.norm` for depth filtering. This eliminates massive Python-level overhead and avoids redundant square root operations, achieving a ~3.2x speedup on large point clouds.
**Action:** Favor vectorized slice assignments over explicit loops for multidimensional masks; use squared distance comparisons to avoid `sqrt` in performance-critical paths.
