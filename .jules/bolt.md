...
**Learning:** Point cloud filtering operations like boundary exclusion and max depth filtering can be massive bottlenecks when implemented with Python loops or non-optimal NumPy functions like `np.linalg.norm`. Using vectorized slice assignments on reshaped views and squared distance comparisons with `np.einsum` provides a ~4.4x speedup for the entire filtering pipeline.
**Action:** Always vectorize filtering logic using NumPy slices and avoid expensive operations like `sqrt` by comparing squared values.
