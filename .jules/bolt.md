...
## 2026-03-05 - [Vectorized Boundary and Squared Distance Filtering]
**Learning:** Nested Python loops for mask manipulation are extremely slow compared to NumPy vectorized slicing. Additionally, calculating Euclidean distance for filtering using `np.linalg.norm` is redundant when squared distance comparisons can achieve the same result without expensive square roots.
**Action:** Always prefer vectorized slicing (reshaping if necessary) for spatial masking; use squared distance (`a**2 + b**2 + c**2 <= threshold**2`) for radius-based filtering.
