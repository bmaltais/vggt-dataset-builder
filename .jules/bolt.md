...
**Learning:** Nested Python loops for boundary filtering on multi-frame masks (S, H, W) are extremely slow for high-resolution images. Reshaping the flat mask into a 3D view and using NumPy slicing (e.g., `mask[:, :threshold, :] = False`) provides a ~8x speedup by pushing the logic into compiled C code.
**Action:** Use NumPy slicing on reshaped views to perform spatial masking instead of nested Python loops.
