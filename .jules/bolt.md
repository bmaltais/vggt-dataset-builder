...
**Learning:** Creating a boundary mask using nested Python loops over frames and rows is extremely slow for high-resolution images (O(S*H)). NumPy's 3D slice assignments allow masking the entire batch's boundaries in a few vectorized operations, providing a ~5x-10x speedup for this phase.
**Action:** Use multi-dimensional NumPy slicing (e.g., `mask[:, :threshold, :] = False`) to zero out image boundaries instead of row-by-row loops.
