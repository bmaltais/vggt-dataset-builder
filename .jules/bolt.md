...
**Learning:** Nested Python loops for boundary masking on large (S*H*W) point clouds are extremely slow (O(S*H)). Reshaping the flattened array to (S, H, W) and using vectorized NumPy slicing provides a ~25x speedup for typical resolutions (e.g., 518x518) while maintaining identical logic.
**Action:** Use NumPy slicing on reshaped arrays to replace coordinate-based Python loops for spatial filtering.
