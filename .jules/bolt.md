...
**Learning:** Applying point cloud filters (boundary, depth) using Python loops over millions of points is extremely slow. Vectorizing boundary filtering with multi-dimensional NumPy slices and replacing `np.linalg.norm` with squared distance comparisons avoids square root overhead and redundant array allocations, providing ~5x speedup.
**Action:** Always vectorize point cloud operations using NumPy slices and in-place boolean operators (`&=`) to minimize overhead on large datasets.
