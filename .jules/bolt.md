...
## 2026-02-18 - [Vectorized Boundary Filtering & In-Place Masking]
**Learning:** Nested Python loops for boundary filtering (e.g., O(S*H*boundary_threshold)) are extremely slow compared to vectorized NumPy slicing. Reshaping a flattened mask to its 3D (S, H, W) dimensions allows for O(1) edge slicing. Additionally, using in-place `&=` operations for combining boolean masks significantly reduces memory pressure by avoiding temporary array allocations.
**Action:** Always prefer vectorized slicing over Python loops for image/mask boundary operations; use in-place logical operators to minimize memory footprint in batch processing.
