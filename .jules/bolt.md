...
**Learning:** Iterating through millions of points in a nested Python loop for boundary filtering is extremely slow. NumPy vectorized slicing and squared distance comparisons (avoiding sqrt) provide massive speedups (3.7x to 6.5x) while reducing memory overhead through in-place operations.
**Action:** Use vectorized slicing for geometric filtering and avoid square root operations in distance calculations when comparing against a threshold.
