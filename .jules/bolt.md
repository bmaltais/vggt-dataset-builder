...
**Learning:** Iteratively setting boundary pixels in a 3D volume (sequence of frames) using nested Python loops is extremely slow (O(S*H)). A 2D boundary mask can be broadcasted across the frame dimension in a single NumPy operation, providing a ~40x speedup for that specific phase.
**Action:** Use NumPy broadcasting to apply spatial masks to multi-frame sequences instead of per-frame loops.
