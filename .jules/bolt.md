...
**Learning:** Using nested Python loops to iterate over rows/frames for masking operations on large NumPy arrays is extremely slow due to Python's loop overhead. Reshaping to 3D `(S, H, W)` and using vectorized slicing allows bulk clearing of boundaries across entire batches in constant time. Additionally, using in-place boolean operators (`&=`) avoids allocating large intermediate boolean arrays, reducing peak memory and improving speed by ~1.6x.
**Action:** Always prefer vectorized slicing for boundary/grid operations; use in-place operators for combining large masks to minimize memory pressure.
