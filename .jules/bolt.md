...
## 2026-02-19 - [Vectorized Boundary Filtering and In-place Masks]
**Learning:** Nested Python loops for boundary filtering (iterating over frames and rows) introduce significant overhead when processing multi-frame sequences, often executing tens of thousands of iterations (O(S*H)). Additionally, repeated boolean mask combinations via `mask = mask & new_mask` create many temporary arrays, increasing memory pressure for large point clouds. Vectorizing boundary filtering with NumPy slicing on a reshaped `(S, H, W)` mask and using in-place `&=` operators significantly improves performance and reduces peak memory.
**Action:** Replace frame/row loops with vectorized slicing for spatial filtering; use in-place `&=` for mask combinations to minimize memory allocations.
