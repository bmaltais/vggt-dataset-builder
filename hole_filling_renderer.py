...
"""
    ...
    def _read_final_color(self) -> np.ndarray:
        """Reads the final rendered RGB image from the GPU as a uint8 array.

        ⚡ Bolt: Optimized readback by using a 3-channel uint8 texture. This reduces
        GPU-to-CPU bandwidth by ~5.3x compared to RGBA float32 and eliminates the
        need for expensive CPU-side scaling, clipping, and type conversion.
        """
        texture = self.final_mask_pass.color_textures[0]
        data = texture.read()
