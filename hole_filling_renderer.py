...
"""
    ...
    def _read_final_color(self) -> np.ndarray:
        texture = self.final_mask_pass.color_textures[0]
        # ⚡ Bolt: Use alignment=1 to ensure we can read RGB data for any width.
        # OpenGL defaults to 4-byte alignment, which would add padding for non-multiple-of-4 widths.
        data = texture.read(alignment=1)
        # ⚡ Bolt: Read directly as uint8 RGB, avoiding float32 conversion and CPU-side scaling.
        # This reduces bandwidth by 5.3x and eliminates expensive math.
