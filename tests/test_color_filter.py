...
class TestColorFilterBlackBackground:
    ...
    def test_filter_black_removes_dark_colors(self):
        ...
        filter_cfg = PointCloudFilter(
            filter_black_bg=True, filter_white_bg=False
        )
    ...
    def test_filter_black_boundary_exact_threshold(self):
        ...
        colors = np.array([
            ...
            [threshold / 3.0, threshold / 3.0, threshold / 3.0],  # Exactly at threshold
...
class TestColorFilterWhiteBackground:
    ...
    def test_filter_white_removes_bright_colors(self):
        ...
        filter_cfg = PointCloudFilter(
            filter_black_bg=False, filter_white_bg=True
        )
    ...
    def test_filter_white_boundary_threshold(self):
        ...
        colors = np.array([
            ...
            [threshold, threshold, threshold],    # All exactly at threshold (filtered)
...
class TestColorFilterBothFilters:
    ...
    def test_both_filters_removes_black_and_white(self):
        ...
        colors = np.array([
            ...
            [white_threshold - 0.01, white_threshold - 0.01, white_threshold - 0.01],  # All < white_threshold (kept)
            [1.0, 1.0, 1.0],                      # All >= white_threshold (filtered)
            [white_threshold, white_threshold, white_threshold],  # All >= white_threshold (filtered)
        ], dtype=np.float32)
...
class TestColorFilterProperties:
    ...
    def test_filter_is_monotonic_with_brightness(self):
        ...
        colors = np.array(
            [[b, b, b] for b in brightnesses], dtype=np.float32
        )
