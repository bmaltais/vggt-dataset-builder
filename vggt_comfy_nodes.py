...
class VGGT_Model_Inference:
    ...
    def infer(
        ...
        **kwargs,
        ...
        if max_depth > 0:
            # ⚡ Bolt: Use squared distance to avoid expensive sqrt from np.linalg.norm.
            # Vectorized calculation on (N, 3) points array.
            dist_sq = (
        ...
        if boundary_threshold > 0:
            ...
            # This replaces nested Python loops that were updating the mask point-by-point.
        ...
        if mask_white_bg:
            # ⚡ Bolt: Optimized white background filter using in-place AND for efficiency.
            valid_mask_all &= ~(
