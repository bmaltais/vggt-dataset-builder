...
class VGGT_Model_Inference:
    ...
    def infer(
        ...
        **kwargs,
        ...
        if max_depth > 0:
            # ⚡ Bolt: Using squared distance comparison avoids the expensive np.linalg.norm (square root) operation.
            sq_depth_all = (
                points_all_frames[:, 0] ** 2
                + points_all_frames[:, 1] ** 2
                + points_all_frames[:, 2] ** 2
            )
            valid_mask_all = valid_mask_all & (sq_depth_all <= max_depth**2)
            print(f"[VGGT] Applied max_depth filter: {max_depth}")

        # Apply boundary filtering to all frames
        if boundary_threshold > 0:
            # ⚡ Bolt: Vectorized boundary filtering using NumPy slicing is significantly faster than nested loops.
