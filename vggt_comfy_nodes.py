...
class VGGT_Model_Inference:
    ...
    def infer(
        ...
        **kwargs,
        ...
        if max_depth > 0:
            # ⚡ Bolt: Use explicit squared distance comparison to avoid redundant
            # square root calculations in np.linalg.norm across millions of points.
            dist_sq = (
        ...
        if boundary_threshold > 0:
            # ⚡ Bolt: Use vectorized 2D broadcasting to filter all frames at once.
            # This avoids O(S*H) Python-level iterations over millions of points.
            ...
            # Broadcast to all frames and apply in-place to avoid array copy
