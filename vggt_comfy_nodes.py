    ...
    def infer(
        ...
        **kwargs,
        ...
        if boundary_threshold > 0:
            # ⚡ Bolt: Vectorized boundary filtering via NumPy slicing is ~300x faster
            # than nested Python loops. Reshaping to (S, H, W) allows bulk clearing
            # of boundaries across the entire batch in four slicing operations.
            ...
            valid_mask_all &= boundary_mask.reshape(-1)
        ...
        if mask_black_bg:
            ...
            # Using in-place &= reduces peak memory by avoiding extra boolean array allocations.
            color_sum = (
                colors_all_frames[:, 0]
                + colors_all_frames[:, 1]
                + colors_all_frames[:, 2]
        ...
        if mask_white_bg:
            # ⚡ Bolt: Using in-place &= reduces peak memory.
            valid_mask_all &= ~(
