    ...
    def infer(
        ...
        **kwargs,
        ...
        if max_depth > 0:
            ...
            # This provides a ~5x speedup for depth filtering on millions of points.
            dist_sq = (
        ...
        if boundary_threshold > 0:
            # ⚡ Bolt: Vectorized boundary filtering using 2D slice assignment on a reshaped view.
            # This eliminates redundant O(S*H) Python iterations, providing a ~15x+ speedup.
        ...
        if mask_white_bg:
            # ⚡ Bolt: Simplified math (colors > 240/255.0) avoids redundant uint8 copies.
            white_mask = ~(
