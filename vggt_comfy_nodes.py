    ...
    def infer(
        ...
        **kwargs,
        ...
        if max_depth > 0:
            ...
            # ⚡ Bolt: Use in-place &= to reduce memory allocations.
        ...
        if boundary_threshold > 0:
            # ⚡ Bolt: Reshape to 3D for vectorized boundary filtering.
            # This replaces nested Python loops with efficient NumPy slicing and avoids
            # extra memory allocations by modifying the flattened mask view in-place.
            boundary_view = valid_mask_all.reshape(S, H, W)
            boundary_view[:, :boundary_threshold, :] = False
            boundary_view[:, -boundary_threshold:, :] = False
            boundary_view[:, :, :boundary_threshold] = False
            boundary_view[:, :, -boundary_threshold:] = False
        ...
        if mask_sky:
            try:
                ...
                for s in range(S):
                    ...
                    # ⚡ Bolt: Directly update the valid_mask_all in-place to avoid
                    # allocating a full sky_mask_all array for the entire sequence.
                    frame_offset = s * H * W
                    valid_mask_all[frame_offset : frame_offset + H * W] &= (
