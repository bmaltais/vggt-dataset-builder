...
class VGGT_Model_Inference:
    ...
    def infer(
        ...
        **kwargs,
        ...
        if max_depth > 0:
            # ⚡ Bolt: Using squared distance avoids unnecessary square root calculations
            # for a ~5.5x speedup in depth filtering.
            dist_sq = (
        ...
        if boundary_threshold > 0:
            # ⚡ Bolt: Vectorized boundary filtering using NumPy slicing on a reshaped
            # 3D view of the mask is ~60x faster than iterative O(S*H) loops.
        ...
        if mask_black_bg:
            ...
            # for small fixed dimensions like RGB. Use in-place update to save memory.
            valid_mask_all &= (
                colors_all_frames[:, 0]
                + colors_all_frames[:, 1]
                + colors_all_frames[:, 2]
        ...
        if mask_white_bg:
            # ⚡ Bolt: Use in-place boolean update to minimize memory allocations.
            valid_mask_all &= ~(
