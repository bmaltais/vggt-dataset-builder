    ...
    def infer(
        ...
        **kwargs,
        ...
        if max_depth > 0:
            ...
            # This provides a ~6x speedup on millions of points.
            dist_sq = (
        ...
        if boundary_threshold > 0:
            # ⚡ Bolt: Vectorized boundary filtering using 3D slicing.
            # This replaces a slow nested Python loop with fast NumPy operations,
        ...
        if mask_black_bg:
            ...
            # for small fixed dimensions like RGB. Use in-place &= for mask update.
            color_sum = (
                colors_all_frames[:, 0]
                + colors_all_frames[:, 1]
                + colors_all_frames[:, 2]
        ...
        if mask_white_bg:
            # ⚡ Bolt: Simplified math and in-place Boolean operator (&=)
            # Use >= 241/255 to match dataset_utils.py and avoid extra mask allocation.
