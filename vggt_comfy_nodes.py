...
class VGGT_Model_Inference:
    ...
    def infer(
        ...
        **kwargs,
        ...
        if max_depth > 0:
            # ⚡ Bolt: Using squared distance avoids expensive square root in np.linalg.norm
            # and in-place operator reduces memory allocations.
            dist_sq = (
        ...
        if boundary_threshold > 0:
            ...
            # is significantly faster than nested loops on large datasets.
        ...
        if mask_black_bg:
            ...
            # for small fixed dimensions like RGB. In-place operator further saves memory.
            color_sum = (
                colors_all_frames[:, 0]
                + colors_all_frames[:, 1]
                + colors_all_frames[:, 2]
        ...
        if mask_white_bg:
            # ⚡ Bolt: Optimized white background filter using in-place operator and
            # thresholds aligned with dataset_utils.py.
