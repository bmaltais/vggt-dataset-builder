...
class VGGT_Model_Inference:
    ...
    def infer(
        ...
        **kwargs,
        ...
        if max_depth > 0:
            # ⚡ Bolt: Optimization - Use squared distance comparison to avoid the overhead
            # of np.linalg.norm (square root) on millions of points.
            valid_mask_all &= (
        ...
        if boundary_threshold > 0:
            # ⚡ Bolt: Vectorized boundary filtering using a 3D view and slice assignments.
            # This replaces iterative Python loops and provides a significant speedup (up to 36x).
        ...
        if mask_white_bg:
            # ⚡ Bolt: Aligned white background threshold with dataset_utils.py (>= 241/255.0)
            # and used in-place Boolean operators for efficiency.
