...
class VGGT_Model_Inference:
    ...
    def infer(
        ...
        **kwargs,
        ...
        if max_depth > 0:
            # ⚡ Bolt: Use squared distance to avoid sqrt overhead (~1.2x speedup) and in-place &=
            dist_sq = (
        ...
        if boundary_threshold > 0:
            # ⚡ Bolt: Vectorized boundary filtering using 3D view slice assignments (~130x speedup)
        ...
        if mask_black_bg:
            ...
            # for small fixed dimensions like RGB. In-place &= reduces memory pressure.
            valid_mask_all &= (
                colors_all_frames[:, 0]
                + colors_all_frames[:, 1]
                + colors_all_frames[:, 2]
        ...
        if mask_white_bg:
            # ⚡ Bolt: Optimized white background filter using in-place &= and simplified math
        ...
        if mask_sky:
            try:
                ...
                # ⚡ Bolt: Use a reshaped view to update valid_mask_all in-place
                mask_view = valid_mask_all.reshape(S, H, W)
                ...
                for s in range(S):
                    ...
                    # ⚡ Bolt: Apply to the corresponding portion of the mask view directly
