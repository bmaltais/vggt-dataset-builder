...
class VGGT_Model_Inference:
    ...
    def infer(
        ...
        **kwargs,
        ...
        if max_depth > 0:
            # ⚡ Bolt: Using explicit squared distance (x^2 + y^2 + z^2) is ~2x faster than
            # np.linalg.norm as it avoids the expensive square root operation.
            dist_sq = (points_all_frames[:, 0]**2 +
                       points_all_frames[:, 1]**2 +
                       points_all_frames[:, 2]**2)
            valid_mask_all = valid_mask_all & (dist_sq <= max_depth**2)
            print(f"[VGGT] Applied max_depth filter: {max_depth}")

        # Apply boundary filtering to all frames
        if boundary_threshold > 0:
            # ⚡ Bolt: Vectorized boundary filtering is ~17x faster than nested Python loops.
            # We reshape to 3D to apply slice assignments across all frames at once.
            valid_mask_all = valid_mask_all.reshape(S, H, W)
            valid_mask_all[:, :boundary_threshold, :] = False
            valid_mask_all[:, -boundary_threshold:, :] = False
            valid_mask_all[:, :, :boundary_threshold] = False
            valid_mask_all[:, :, -boundary_threshold:] = False
