...
class VGGT_Model_Inference:
    ...
    def infer(
        ...
        **kwargs,
        ...
        # ⚡ Bolt: Use in-place &= to minimize temporary boolean array allocations.
        valid_mask_all = conf_all_frames >= conf_threshold_value
        valid_mask_all &= conf_all_frames > 1e-5

        # Apply max depth filtering to all frames
        if max_depth > 0:
            # ⚡ Bolt: Use squared distance to avoid expensive np.linalg.norm (sqrt overhead).
            # dist_sq <= max_depth**2 is mathematically equivalent to dist <= max_depth.
            # Using np.einsum is more memory-efficient than np.sum(points**2).
        ...
        if boundary_threshold > 0:
            # ⚡ Bolt: Vectorize boundary filtering using slice assignment on a 3D view.
            # This replaces a slow O(S*H*W) nested loop with highly efficient O(1) NumPy operations.
        ...
        if mask_black_bg:
            ...
            # for small fixed dimensions like RGB. Use in-place &= to reduce memory pressure.
            color_sum = (
                colors_all_frames[:, 0]
                + colors_all_frames[:, 1]
                + colors_all_frames[:, 2]
        ...
        if mask_white_bg:
            # ⚡ Bolt: Vectorized color thresholding with in-place &=.
            # colors > 240/255 is equivalent to colors >= 241/255.
            white_mask = ~(
