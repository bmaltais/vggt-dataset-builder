    ...
    def infer(
        ...
        **kwargs,
        ...
        if max_depth > 0:
            ...
            # is ~3.5x faster than np.linalg.norm as it avoids the expensive square root.
            depth_all_sq = (
                points_all_frames[:, 0] ** 2
                + points_all_frames[:, 1] ** 2
                + points_all_frames[:, 2] ** 2
            )
            valid_mask_all = valid_mask_all & (depth_all_sq <= max_depth**2)
            print(f"[VGGT] Applied max_depth filter: {max_depth}")

        # Apply boundary filtering to all frames
        if boundary_threshold > 0:
            # ⚡ Bolt: Vectorized boundary filtering is ~27x faster than iterative loops
            # for large frame sequences. Using 3D view for slice assignments.
            mask_view = valid_mask_all.reshape(S, H, W)
            mask_view[:, :boundary_threshold, :] = False
            if boundary_threshold < H:
                mask_view[:, -boundary_threshold:, :] = False
            mask_view[:, :, :boundary_threshold] = False
            if boundary_threshold < W:
                mask_view[:, :, -boundary_threshold:] = False
        ...
        if mask_white_bg:
            # ⚡ Bolt: Aligning white background filter threshold with dataset_utils.py (>= 241/255.0)
            # for consistency and correctness across the pipeline.
