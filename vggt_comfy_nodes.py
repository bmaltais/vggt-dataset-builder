    ...
    def infer(
        ...
        **kwargs,
        ...
        if max_depth > 0:
            ...
            # ⚡ Bolt: In-place mask combination reduces memory overhead and improves speed.
        ...
        if boundary_threshold > 0:
            # ⚡ Bolt: Vectorized boundary filtering by reshaping the mask to 3D.
            # This avoids nested Python loops and redundant mask allocations.
            valid_mask_3d = valid_mask_all.reshape(S, H, W)
            valid_mask_3d[:, :boundary_threshold, :] = False
            valid_mask_3d[:, -boundary_threshold:, :] = False
            valid_mask_3d[:, :, :boundary_threshold] = False
            valid_mask_3d[:, :, -boundary_threshold:] = False
        ...
        if mask_black_bg:
            ...
            # ⚡ Bolt: In-place mask combination reduces memory pressure by avoiding temporary allocations.
