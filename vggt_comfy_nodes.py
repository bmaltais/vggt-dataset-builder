...
class VGGT_Model_Inference:
    ...
    def infer(
        ...
        **kwargs,
        ...
        if max_depth > 0:
            # ⚡ Bolt: Fast depth filtering using squared distance avoids expensive np.linalg.norm
            # square roots, providing a ~1.1x speedup for this specific check.
            dist_sq = (
        ...
        if boundary_threshold > 0:
            # ⚡ Bolt: Vectorized boundary filtering by broadcasting a 2D mask to all frames
            # avoids slow nested Python loops, providing ~40x speedup for this operation.
            ...
            # Use in-place &= to combine masks without extra allocations
        ...
        if mask_black_bg:
            ...
            # for small fixed dimensions like RGB. Explicit casting to uint32 is not needed here
            # as colors_all_frames are already float32.
            color_sum = (
                colors_all_frames[:, 0]
                + colors_all_frames[:, 1]
                + colors_all_frames[:, 2]
        ...
        if mask_white_bg:
            # ⚡ Bolt: In-place mask combination reduces memory pressure
            valid_mask_all &= ~(
        ...
        if mask_sky:
            try:
                ...
                # ⚡ Bolt: In-place mask combination minimizes memory allocations
