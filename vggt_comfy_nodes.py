...
class VGGT_Model_Inference:
    ...
    def infer(
        ...
        **kwargs,
        ...
        if max_depth > 0:
            # ⚡ Bolt: Use squared distance with np.einsum for depth filtering.
            # This avoids the expensive square root operation in np.linalg.norm.
            dist_sq = np.einsum("ij,ij->i", points_all_frames, points_all_frames)
            valid_mask_all &= dist_sq <= max_depth**2
            print(f"[VGGT] Applied optimized max_depth filter: {max_depth}")
        ...
        if boundary_threshold > 0:
            ...
            # This replaces a slow per-row nested loop with efficient bulk memory clearing.
            ...
            mask_view[:, :, -boundary_threshold:] = False
            print(
                f"[VGGT] Applied vectorized boundary_threshold filter: {boundary_threshold}px"
            )
        ...
        if mask_black_bg:
            ...
            # Use in-place &= to minimize temporary boolean array allocations
            valid_mask_all &= color_sum >= (16 / 255.0)
            print(f"[VGGT] Applied mask_black_bg filter")

        if mask_white_bg:
            # ⚡ Bolt: Use in-place &= and bitwise NOT to reduce peak memory usage
            valid_mask_all &= ~(
