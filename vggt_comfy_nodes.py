...
class VGGT_Model_Inference:
    ...
    def infer(
        ...
        **kwargs,
        ...
        if max_depth > 0:
            # ⚡ Bolt: Using squared distance (einsum) is ~4x faster than np.linalg.norm
            # as it avoids square root overhead and redundant array allocations.
        ...
        if boundary_threshold > 0:
            # ⚡ Bolt: Vectorized boundary filtering is ~40x faster than nested loops.
            # Using a 3D view and slice assignments enables in-place updates.
        ...
        if mask_black_bg:
            ...
            # ⚡ Bolt: In-place mask update reduces peak memory usage
            valid_mask_all &= color_sum >= (16 / 255.0)
            print(f"[VGGT] Applied mask_black_bg filter")

        if mask_white_bg:
            # ⚡ Bolt: Simplified math and in-place update for white background filter
            valid_mask_all &= ~(
