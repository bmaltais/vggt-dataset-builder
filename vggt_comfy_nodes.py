...
class VGGT_Model_Inference:
    ...
    def infer(
        ...
        **kwargs,
        ...
        if max_depth > 0:
            # ⚡ Bolt: Use squared distance to avoid expensive np.linalg.norm (sqrt)
            # and in-place assignment to reduce temporary allocations.
            dist_sq = (
        ...
        if boundary_threshold > 0:
            ...
            # This replaces the slow nested loop (~300x speedup for 1080p).
        ...
        if mask_white_bg:
            # ⚡ Bolt: Combined threshold check with in-place bitwise AND reduces allocations.
            threshold = 240 / 255.0
            valid_mask_all &= ~(
