...
class VGGT_Model_Inference:
    ...
    def infer(
        ...
        **kwargs,
        ...
        if max_depth > 0:
            # ⚡ Bolt: Using squared distance (x^2 + y^2 + z^2) avoids expensive np.linalg.norm (sqrt).
            # We use in-place &= to minimize temporary boolean array allocations.
            dist_sq = (
        ...
        if boundary_threshold > 0:
            # ⚡ Bolt: Vectorized boundary filtering is ~2x faster than nested loops.
        ...
        if mask_black_bg:
            ...
            # for small fixed dimensions like RGB. In-place &= reduces memory overhead.
            color_sum = (
                colors_all_frames[:, 0]
                + colors_all_frames[:, 1]
                + colors_all_frames[:, 2]
        ...
        if mask_white_bg:
            # ⚡ Bolt: Avoid ~(A & B & C) double-negation by combining filters directly.
            # Threshold matches dataset_utils.py (>= 241/255.0).
